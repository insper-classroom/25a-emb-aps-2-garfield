 ...
 */
 ...
 static const uint BTN_PIN[BTN_COUNT] = {2, 3, 4, 5, 6, 8, 9, 10};
 ...
 void uart_task(void *pv) {
     ...
     while (1) {
         if (xQueueReceive(xQueueADC, &recv, portMAX_DELAY)) {
            ...
            uint8_t sync = 0xFF;
            uint8_t val_lo = recv.val & 0xFF;
            uint8_t val_hi = (recv.val >> 8) & 0xFF;
            
            // Envio dos bytes na ordem correta
            putchar_raw(sync);       // Byte de sincronização
            putchar_raw(recv.axis);       // Axis
            putchar_raw(val_lo);     // Byte menos significativo do valor
            putchar_raw(val_hi);     // Byte mais significativo do valor
        
            // Debug
            printf("[UART] sync=0x%02X, axis=0x%d, val_lo=0x%02X, val_hi=0x%02X\n",
                   sync, recv.axis, val_lo, val_hi);
